...
const includes: Record<string, object> = {
  projects: {
    area: true,
    _count: {
      select: { modules: true, tasks: true, assets: true, ideas: true },
    },
  },
...
const migratedResources = ["areas", "projects", "modules", "tasks"];
const createdAtResources = ["alerts", "inbox-items", "activity-log"];
...
function orderByFor(resource: string) {
  return createdAtResources.includes(resource) ? { createdAt: "desc" } : { updatedAt: "desc" };
}
...
export async function POST(req: NextRequest, { params }: Context) {
  ...
  try {
    const { resource, slug = [] } = await params;
    if (migratedResources.includes(resource)) {
    ...
    if (id && action === "freeze") {
      if (!body.frozenReason) return error("El motivo de congelamiento es obligatorio.");
      return NextResponse.json(
        await m.update({
          where: { id },
          data: {
    ...
    if (id && action === "unfreeze") {
      if (body.status === "active" && !body.nextAction) {
        return error("Todo proyecto activo debe tener una próxima acción concreta.");
      }
      return NextResponse.json(
        await m.update({
          where: { id },
    ...
    if (id && action === "complete") {
      return NextResponse.json(await m.update({ where: { id }, data: { status: "completed", completedAt: new Date() } }));
    }
    ...
    if (id && action === "convert-to-project") {
      ...
      await db.idea.update({ where: { id }, data: { status: "promoted", projectId: project.id } });
      return NextResponse.json(project, { status: 201 });
    }

    return NextResponse.json(await m.create({ data: parse(resource, body) }), { status: 201 });
  } catch (e) {
