...
export async function GET(request: NextRequest) {
  const auth = await requireApiSession(); if (auth) return auth; const query = request.nextUrl.searchParams; const status = query.get("status");
  ...
  return NextResponse.json(await getPrisma().task.findMany({ where: { areaId: query.get("areaId") ?? undefined, projectId: query.get("projectId") ?? undefined, moduleId: query.get("moduleId") ?? undefined, status: status ? status as never : { not: "discarded" } }, include, orderBy: [{ status: "asc" }, { dueDate: "asc" }, { createdAt: "asc" }] }));
}
