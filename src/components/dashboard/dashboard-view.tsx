...
export function DashboardView({ data }: { data: { tasks: any[], projects: any[], dues: any[], reviews: any[] } }) {
  ...
  const priorityTasks = [...openTasks]
    .filter(
      (task) =>
        ...
        task.isCritical ||
        task.status === "blocked" ||
        Boolean(task.dueDate && days(task.dueDate) <= 0),
    )
    .sort(
      (a, b) =>
        taskScore(b) - taskScore(a) ||
        (a.dueDate ? days(a.dueDate) : 999) -
          (b.dueDate ? days(b.dueDate) : 999),
    );
  const todayTasks = openTasks
    .filter(
      (task) =>
        task.isToday || Boolean(task.dueDate && days(task.dueDate) === 0),
    )
  ...
  const blockedProjects = data.projects.filter((project) =>
    ["blocked"].includes(project.status),
  );
  const waitingItems = [
    ...blockedTasks.map((task) => ({
      id: `task-${task.id}`,
      title: task.title,
      subtitle: projectById.get(task.projectId || "")?.name || "Inbox",
      status: task.status,
  ...
  const calendarItems = [
    ...openTasks
      ...
      .map((task) => ({
        ...
        date: task.dueDate!,
        kind: "Tarea",
        tone: task.status === "blocked" ? "critical" : task.priority,
      })),
    ...data.dues
      ...
      .map((due) => ({
        ...
        date: due.dueDate,
        kind: "Vencimiento",
        tone: days(due.dueDate) < 0 ? "critical" : "high",
      })),
    ...data.reviews
      ...
      .map((review) => ({
        ...
        kind: "Revisión",
        tone: days(review.nextReviewDate) < 0 ? "critical" : "medium",
      })),
  ];
    ...
    >
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 truncate text-xs text-slate-500">
          {project?.name || "Inbox"}
          {task.dueDate ? ` · ${overdue ? "Vencida" : fmt(task.dueDate)}` : ""}
        </p>
      </div>
        ...
        {cells.map((cell, index) => {
          ...
          return (
            <button
              ...
              type="button"
              disabled={!cell}
              onClick={() => cell && setSelectedDate(dateKey(cell))}
              className={`relative h-11 rounded-md border text-xs transition ${
                !cell
