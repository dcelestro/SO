...
export function ExplorerFormDialog({ kind, open, onOpenChange, entity, areaId, projectId, onSaved }: { kind: Kind; open: boolean; onOpenChange: (open: boolean) => void; entity?: Entity; areaId?: string; projectId?: string; onSaved: (type: Kind, id: string) => void }) {
  ...
  async function save() {
    ...
    const payload = isArea ? { name, description, color, status } : { name, description, status, priority, nextAction, blockedReason, progressPercentage: Number(progress), areaId: areaId ?? current?.areaId, ...(kind === "module" ? { projectId: projectId ?? current?.projectId } : {}) };
    try {
      const resource = kind === "area" ? "areas" : kind === "project" ? "projects" : "modules";
      const response = await fetch(`/api/${resource}${entity ? `/${entity.id}` : ""}`, { method: entity ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      const body = await response.json(); if (!response.ok) throw new Error(body.error ?? "No se pudo guardar.");
      onSaved(kind, body.id); onOpenChange(false);
  ...
  </div><DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Cancelar</Button><Button onClick={save} disabled={saving}>{saving ? "Guardando…" : "Guardar"}</Button></DialogFooter></DialogContent></Dialog>;
}
