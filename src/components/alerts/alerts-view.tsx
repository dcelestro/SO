          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-500">
                <span>{typeLabels[item.type] || item.type}</span><span>·</span><span>{item.context || "Sin contexto"}</span>{item.date ? <><span>·</span><span>{fmt(item.date)}</span></> : null}
              </div>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2 sm:justify-end">
            <Status value={item.severity} />{item.generated ? <span className="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-600">Automática</span> : <Status value={item.status} />}
          </div>
        </div>
      </CardHeader>
...
function Metric({ label, value, alert }: { label: string; value: number; alert?: boolean }) {
  return <div className="rounded-xl border bg-white p-4"><p className={`text-2xl font-semibold ${alert && value ? "text-red-600" : "text-slate-900"}`}>{value}</p><p className="text-xs text-slate-500">{label}</p></div>;
}
